...
def make_new_handle(conn):
    # last_handle = Handles.objects.last()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM handles ORDER BY id DESC LIMIT 1")

    # Fetch the result
    last_handle = cursor.fetchone()
...
def mint_handle_main_function(request, landing_page_url=None):
    ...
    # attempt coonection to mariadb handle database
    try:
        # Connect to MariaDB
    ...
    if type(res['url']) is str:
        ...
        # Execute query
        cursor.execute("SELECT * FROM Handles where data=?", (res['url'],))
        ...
        if len(qs):
            ...
            result = mint_new_handle(make_new_handle(conn), res['url'])
    ...
    if type(res['url']) is list:
        ...
        for url in urls:
            ...
            if qs.exists():
                ...
                result = mint_new_handle(make_new_handle(conn), 'url')
